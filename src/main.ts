...
export async function run(): Promise<void> {
	try {
		...
		const serviceAccountJson = fromServiceAccountJsonFile(
			JSON.parse(ycSaJsonCredentials),
		);
		...
		const middleware: FinalizeRequestMiddleware<any, any> = (next) => {
			return async (args) => {
				...
				args.request.headers["X-YaCloud-SubjectToken"] =
	...
	try {
		...
		for (const include of inputs.include) {
			...
			for (const match of matches) {
				const res = !patterns
					.map((p) => minimatch(match, p, { matchBase: true }))
					.some((x) => x);
				if (res) {
